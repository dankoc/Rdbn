...
#' centered on a 
read_genomic_data <- function(bed, bigwig_plus, bigwig_minus, as_matrix= TRUE, window_sizes= c(10, 50, 500), half_nWindows= c(20, 20, 20)) {

  stopifnot(NROW(window_sizes) == NROW(half_nWindows))
  zoom<- list(as.integer(window_sizes), as.integer(half_nWindows))

  dat <- .Call("get_genomic_data_R", as.character(bed[,1]), as.integer((bed[,3]+bed[,2])/2), as.character(bigwig_plus), as.character(bigwig_minus), zoom, PACKAGE= "Rdbn")
